...
**Goal:** Repair the locally actionable SEO and presentation issues from the DQR.GG audit, while preserving evidence-gated publication and producing directly reviewable local URLs.
...
- Canonical host is `https://dqr.gg`; old `.wiki` host redirects permanently while preserving path and query string.
...
- [x] Add a test that asserts the robots sitemap URL is exactly `https://dqr.gg/sitemap.xml` and contains no `//sitemap` path.
...
### Task 2: Redirect the legacy `.wiki` host
...
**Interfaces:**
- `getLegacyHostRedirect(host: string | null, pathname: string, search: string): string | null` returns a canonical `https://dqr.gg` URL for `dungeonquestrebornguide.wiki` and `www.dungeonquestrebornguide.wiki`, otherwise `null`.
- `proxy.ts` returns a permanent redirect for legacy hosts and leaves canonical/local/preview hosts untouched.

- [x] Add failing tests for both legacy hosts, port-suffixed local requests, query preservation, and canonical-host passthrough.
...
- [x] Add `proxy.ts` that reads the request host and returns a 308 redirect only for the two legacy hosts.
...
- [x] Run `NEXT_PUBLIC_SITE_URL=https://dqr.gg npm.cmd run build` using the Windows-compatible environment assignment.
